...
TEST_F(Ring_BufferTest, BufferEmpty)
{
    ...
    EXPECT_EQ(true, rb_is_empty(&ring_buffer));
    EXPECT_EQ(false, rb_is_full(&ring_buffer));
}
...
TEST_F(Ring_BufferTest, BufferFill)
{
    ...
    EXPECT_EQ(true, rb_is_full(&ring_buffer));
    EXPECT_EQ(false, rb_is_empty(&ring_buffer));
...
TEST_F(Ring_BufferTest, BufferFillSafe)
{
    uint8_t testData[BUF_SIZE];
    GenerateTestData(testData, sizeof(testData));
    ...
    uint32_t difference = 0;
    for (uint32_t i = 0; i < BUF_SIZE; ++i)
        difference += testData[BUF_SIZE - 1 - i] - rb_safe_remove(&ring_buffer);
...
TEST_F(Ring_BufferTest, BufferOverfillByOneDestructive)
{
    ...
    for (uint32_t i = 0; i < BUF_SIZE; ++i)
    {
        EXPECT_EQ(true, rb_insert(&ring_buffer, testData[i]));
    }

    // Add the extra one value
    EXPECT_EQ(true, rb_is_full(&ring_buffer));
    EXPECT_EQ(false, rb_insert(&ring_buffer, testData[BUF_SIZE + 1]));
    EXPECT_EQ(true, rb_is_full(&ring_buffer));
...
TEST_F(Ring_BufferTest, BufferOverfillByOneSafe)
{
    ...
    // Add the extra one value
    EXPECT_EQ(true, rb_is_full(&ring_buffer));
    EXPECT_EQ(false, rb_safe_insert(&ring_buffer, testData[BUF_SIZE + 1]));
    EXPECT_EQ(true, rb_is_full(&ring_buffer));
...
TEST_F(Ring_BufferTest, Peek)
{
    ...
    {
        rb_safe_insert(&ring_buffer, testData[i]);
        EXPECT_EQ(testData[i], rb_peek(&ring_buffer));
    }
}
